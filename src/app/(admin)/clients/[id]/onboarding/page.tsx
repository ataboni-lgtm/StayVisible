...
export default async function OnboardingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="@container/page mx-auto max-w-4xl"><PageHeader eyebrow="LinkedIn voice onboarding" title={`Help us sound like ${clientDisplayName(client)}`} description="These answers shape every future draft. The goal is not a perfect persona. It’s a recognizable one." /><OnboardingForm clientId={client.id} /></div>;
}
